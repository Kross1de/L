...
private:
	...
	bool isType(TokenType t) {
		return t == TokenType::U8 || t == TokenType::U16 || t == TokenType::U32 || t == TokenType::U64 || t == TokenType::INT || t == TokenType::FLOAT;
	}
		...
		if (currentToken.type == TokenType::FN) {
			...
			return new VarDeclNode(varType, varName, initializer);
		} else {
			ASTNode* left = expr();
			if (currentToken.type == TokenType::EQUAL) {
				advance();
				...
				if (auto id = dynamic_cast<IdentifierNode*>(left)) {
					...
					throw std::runtime_error("Left side of assignment must be an identifier at line " +
						std::to_string(currentToken.line) + ", column " +
						std::to_string(currentToken.column));
				}
			} else {
